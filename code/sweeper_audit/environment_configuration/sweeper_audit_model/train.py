...
def parse_args():
    parser = ArgumentParser(description='Sweeper Audit System Model Training')
    # model and dataset
    parser.add_argument('--model', type=str, default="ERFNet", help="model name: (default ENet)")
    parser.add_argument('--dataset', type=str, default="sweeper", help="dataset:= sweeper")
    ...
    parser.add_argument('--classes', type=int, default=11,
                        help="the number of classes in the dataset. 4 for sweeper")
    parser.add_argument('--train_type', type=str, default="train",
                        help="ontrain for training on train set, ontrainval for training on train+val set")
    parser.add_argument('--build_new_dataset', type=bool, default=False, help="whether to update the data in the current dataset")
...
def train_model(args):
    ...
    model = build_model(args.model, num_classes=args.classes) #104行关注分类数量
    ...
    datas, trainLoader, valLoader = build_dataset_train(args.dataset, args.build_new_dataset, input_size, args.batch_size, args.train_type,
    ...
    elif args.dataset == 'sweeper' and args.use_label_smoothing:
        criteria = CrossEntropyLoss2dLabelSmooth(weight=weight, ignore_label=ignore_label)
    else:
        raise NotImplementedError(
            "This repository now supports sweeper, %s is not included" % args.dataset)
    ...
    args.savedir = (args.savedir + args.dataset + '/' + args.model + 'bs'
                    + str(args.batch_size) + 'gpu' + str(args.gpu_nums) + "_" + str(args.train_type) + "_"
...
if __name__ == '__main__':
    ...
    if args.dataset == 'sweeper':
        ...
        raise NotImplementedError(
            "This repository now supports two datasets: sweeper, %s is not included" % args.dataset)
