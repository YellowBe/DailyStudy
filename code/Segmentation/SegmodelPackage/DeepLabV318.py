...
class ResNet_Bottleneck_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 50:
            ...
            resnet.load_state_dict(torch.load("./model/resnet50-19c8e357.pth"))
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("./model/resnet152-b121ed2d.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
class ResNet_BasicBlock_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 18:
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("./model/resnet34-333f7ec4.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
class DeepLabV3(nn.Module):
    def __init__(self, classes=19):
        ...
        self.backbone = 'resnet18os16'
        self.num_classes = classes
        if self.backbone == 'resnet34os8':
            self.resnet = ResNet34_OS8()
        if self.backbone == 'resnet34os16':
            self.resnet = ResNet34_OS16()
        elif self.backbone == 'resnet18os8':
            self.resnet = ResNet18_OS8()
        elif self.backbone == 'resnet18os16':
...
if __name__ == '__main__':
    ...
    model = DeepLabV3(classes=19).to(device)
